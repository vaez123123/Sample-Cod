...
using IPSO.CMP.CommonFunctions.DataBaseMODEL;
...
namespace IPSO.CMP.CommonFunctions.Functions
{
    public class ReaderFunc
    {
        ...
        public void readCommonData(CommonLists Lst, int counterCoil, int counterCoilRelease, int counterCoilReleaseOtherSt
           , string widAtt, string TksAtt, string TksOutAtt, string Prog,string SurfaceRoughAtt, string trim, string oil)
        {

            readParameter(widAtt, TksAtt, TksOutAtt , Prog,SurfaceRoughAtt, trim, oil);

            TimeParameter.timereadCoil.Start();
            readCoils(Lst.Coils, counterCoil);
            ...
            //readGroupDef(Lst.GroupDefs, Lst.ProgEfrazes);
        ...
        private void readParameter(string widAtt, string TksAtt, string TksOutAtt, string Prog, string SurfaceRoughAtt, string trim, string oil)
        {
            Parameter.InitializeLogicals(1, 2, 3, 4, 5);
            Parameter.InitiliazeAttributes("", widAtt, TksAtt, TksOutAtt, Prog, SurfaceRoughAtt, trim, oil);
            ...
            //SRM
            Parameter.IntializeTypeMis("MALOTH", "MALEXP", "MALFEX", "MB");
        }

        private void readCoils(List<Coil> Coils, int counterCoil)
        {
            dt = new DataTable();
            string comment = string.Format(@"select c.NUM_PROD_SNAMU,
                                                  ...
                                                  (select to_char(Max(Dat_Calde),'yyyy/mm/dd','NLS_CALENDAR=persian') 
                                                   ...
                                                   nvl((select to_date(Max(Dat_Calde))
                                                            from AAC_CALENDAR_DETAILS ACD
                                                            Where ACD.CALNR_LKP_COD_CAL_CALNR = 'A'
                                                            And to_char(Dat_Calde, 'yyyy', 'NLS_CALENDAR=persian') 
                                                            || LPAD((Num_Week_Year_Calde), 2, '0') = os.NUM_DAT_LAST_ORDER_OSTIF),o.dat_dlv_order_orinf
        ...
        private void readReleaseCoil(List<ReleaseSched> ReleaseSchedsLoc, List<CoilRelease> CoilReleases, int counterCoilRelease, int NumStation)
        {
            ...
            string comment = string.Format(@"select distinct (c.NUM_PROD_SNAMU),
                                                   c.opt_snapshot_mu_id,
                                                   c.Cod_Order_Snamu,
                                                   c.Wid_Prod_Snamu,
                                                   c.Tks_Prod_Snamu,
                                                ...
                                                from cmp.cmp_opt_release_schedules t, cmp.cmp_Opt_Snapshot_Mus c,
                                                     cmp_opt_order_infos  o, cmp_opt_order_station_infos  os
        ...
        private void readProgEfraz(List<ProgEfraz> ProgEfrazes, List<Coil> Coils, List<CoilRelease> CoilReleases, List<CoilRelease> CoilReleasesOtherSt)
        {
            ...
            string comment = string.Format(@"select t.cod_seprate_prog_sptyp,
                                          ...
                                          from mas.mas_sepration_types t,
                                            mas_cmp_program_type_def_viw w
                                         where t.STATN_BAS_STATION_ID ={0}
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if (z == int.Parse(dr["cod_seprate_prog_sptyp"].ToString()))
                {
                    ...
                    {
                        string Afrazlocal = dr["nam_latin_sptyp"].ToString().ToUpper();

                        if (Afrazlocal == Parameter.TrimAttribute)
                        {
                            ProgEfrazes.Last().Trim.Add(Convert.ToInt32(dr["val_attribute_sptyp"]));
                        }

                        else if (Afrazlocal == Parameter.OilAttribute)
                        {
        ...
        private void readTksJump(List<TksJump> TksJumps)
        {
            ...
            string comment = string.Format(@"select t.tks_coil_befor_from_tkjum,
                                               ...
                                               (select p.nam_program_type_pgtyp from mas_bas_program_type_defs p
        ...
        private void readMaxValue()
        {
            ...
            string comment = string.Format(@"select max(NUM_DAT_LAST_ORDER_OSTIF) AS maxDatLast,
                                                   min(NUM_DAT_LAST_ORDER_OSTIF) as minDatLast ,
                                              ...
                                              from (select nvl((select to_date(Max(Dat_Calde))
                                                        from AAC_CALENDAR_DETAILS ACD
                                                        Where ACD.CALNR_LKP_COD_CAL_CALNR = 'A'
                                                        And to_char(Dat_Calde, 'yyyy', 'NLS_CALENDAR=persian') ||
                                                            LPAD((Num_Week_Year_Calde), 2, '0') =
                                                            os.NUM_DAT_LAST_ORDER_OSTIF),o.dat_dlv_order_orinf) as NUM_DAT_LAST_ORDER_OSTIF,
                                                      ...
                                                      from cmp.cmp_opt_snapshot_mus    c,
                                                           cmp_opt_order_infos         o,
                                                           cmp_opt_order_station_infos os
        
                                                     where c.ORINF_OPT_ORDER_INFO_ID = o.opt_order_info_id
                                                       and o.opt_order_info_id = os.Orinf_Opt_Order_Info_Id
                                                       and (c.statn_bas_station_id_uses = {0} or
                                                           c.statn_bas_station_id_uses_a = {0} or
                                                           c.statn_bas_station_id_uses_b = {0} or
                                                           c.statn_bas_station_id_uses_c = {0} or
                                                           c.statn_bas_station_id_uses_d = {0})
                                                       and os.Statn_Bas_Station_Id = {0}
                                                       and c.runth_opt_run_time_hist_id = {1}
                                                       and o.RUNTH_OPT_RUN_TIME_HIST_ID = {1}
        ...
        private void readCapPlan(ref List<CapPlan> CapPlans, List<Coil> Coils, ref List<CapPlan> CapPlansCurr, List<CapPlanUpDate> CapPlanUpDates
            , List<DBDCampPlan> DBDCampPlans)
        {
            dt = new DataTable();


            string comment = string.Format(@"select t.val_net_cap_cappa * 1000 as  val_net_cap_cappa,
                                               t.dat_plan_cappa,
                                               t.cod_prod_cappa,
                                               t.cod_group_dbd_cappa,
                                               t.flg_type_dbd_output_cappa
                                          from cmp.cmp_opt_input_cap_plans t
                                         
                                         where trunc(t.dat_plan_cappa) >=  trunc (sysdate)
                                           and t.statn_bas_station_id ={0}
                                           and   t.runth_opt_run_time_hist_id = {1}
                                            order by t.dat_plan_cappa , t.cod_prod_cappa", RunInformation.NumStation, RunInformation.RunId);
            ...
            foreach (DataRow dr in dt.Rows)
            {
                if (double.Parse(dr["flg_type_dbd_output_cappa"].ToString()) == 1)
                {
                    local = false;
                    addCapPlan(dr, CapPlans);
                    addCapPlan(dr, CapPlansCurr);
            ...
            if (local == true)
            {
                ...
                List<int> pfCoil = new List<int>();
                pfCoil = Coils.Select(a => a.PfId).Distinct().ToList();

                for (int j = 0; j < pfCoil.Count; j++)
                {
                    for (int i = 0; i <= Parameter.DurationPlanning; i++)
                    {
                        localCap = new CapPlan();
                        localCap.DatePlan = Status.CurrTime.AddDays(i).Date;
                        localCap.NetValuePf = 0;
                        localCap.PfId = pfCoil[j];
                        CapPlans.Add(localCap);
                    }
                }
            }
            else
            {
                ...
                pfCoil = Coils.Select(a => a.PfId).Distinct().ToList();
                CapPlans.RemoveAll(c => !pfCoil.Contains(c.PfId));

                Parameter.DurationPlanning = (CapPlans.Max(c => c.DatePlan).Date - Status.CurrTime.Date).Days;

                for (int d = 0; d <= Parameter.DurationPlanning; d++)
                {
                    for (int n = 0; n < d; n++)
                    {
                        pfCapLoc1 = CapPlans.Where(b => (b.DatePlan.Date - Status.CurrTime.Date).Days == n).Select(a => a.PfId).ToList();
                        pfCapLoc2 = CapPlans.Where(b => (b.DatePlan.Date - Status.CurrTime.Date).Days == d).Select(a => a.PfId).ToList();

                        foreach (int i in pfCapLoc1)
                        {
                            if (pfCapLoc2.Contains(i) != true)
                            {
                                CapPlan localCap = new CapPlan();
                                localCap.DatePlan = Status.CurrTime.AddDays(d).Date;
                                localCap.NetValuePf = 0;
                                localCap.PfId = i;
                                CapPlans.Add(localCap);
                            }
                    ...
                    if (d == 0)
                    {
                        List<int> loclpf = CapPlans.Where(b => (b.DatePlan.Date - Status.CurrTime.Date).Days == d).Select(a => a.PfId).ToList();

                        foreach (int i in pfCoil)
                        {
                            if (loclpf.Contains(i) != true)
                            {
                                CapPlan localCap = new CapPlan();
                                localCap.DatePlan = Status.CurrTime.AddDays(d).Date;
                                localCap.NetValuePf = 0;
                       ...
                       CapPlans.RemoveAll(a => !pfCoil.Contains(a.PfId));
                    }
                }
            }
            CapPlans = CapPlans.OrderBy(a => a.DatePlan).ThenBy(a => a.PfId).ToList();

            List<int> PfNew = CapPlans.Select(c => c.PfId).Distinct().ToList();

            foreach (var item in PfNew)
            {
                CapPlanUpDate b = new CapPlanUpDate();
                b.PfId = item;
                b.RespondTemValueobj = 0;
                CapPlanUpDates.Add(b);
            }

            DateTime maxDate = CapPlans.Max(a => a.DatePlan).Date;

            for (DateTime d = Status.CurrTime.Date; d <= maxDate; d = d.AddDays(1))
            {
                int index = CapPlans.FindIndex(a => a.NetValuePfFix > 0 && a.DatePlan == d);
                foreach (var item in CapPlans.Where(a => a.DatePlan == d))
                {
                    if (index != -1)
                        item.FlgInactive = 1;
                    else
            ...
            if (local == true)
            {
                CapPlan localCap = new CapPlan();
                List<int> pfCoil = new List<int>();
                pfCoil = Coils.Select(a => a.PfId).ToList();
                pfCoil = pfCoil.Distinct().ToList();
            ...
            else
            {
                ...
                CapPlansCurr.RemoveAll(c => !pfCoil.Contains(c.PfId));

                Parameter.DurationPlanning = (CapPlansCurr.Max(c => c.DatePlan).Date - Status.CurrTime.Date).Days;

                for (int d = 0; d <= Parameter.DurationPlanning; d++)
                {
                    //List<CapPlan> capLocl = new List<CapPlan>();
                    //List<CapPlan> capLoc2 = new List<CapPlan>();
                    ...
                    for (int n = 0; n < d; n++)
                    {
                        ...
                        //capLocl = CapPlansCurr.Where(b => (b.DatePlan.Date - Status.CurrTime.Date).Days == n).ToList();
                        pfCapLocl = CapPlansCurr.Where(b => (b.DatePlan.Date - Status.CurrTime.Date).Days == n).Select(a => a.PfId).ToList();

                        //capLoc2 = CapPlansCurr.Where(b => (b.DatePlan.Date - Status.CurrTime.Date).Days == d).ToList();
                        pfCapLoc2 = CapPlansCurr.Where(b => (b.DatePlan.Date - Status.CurrTime.Date).Days == d).Select(a => a.PfId).ToList();

                        foreach (int i in pfCapLocl)
                        {
                            if (pfCapLoc2.Contains(i) != true)
                            {
                                CapPlan localCap = new CapPlan();
                                CapPlanUpDate localc = new CapPlanUpDate();
                                localCap.DatePlan = Status.CurrTime.AddDays(d).Date;
                                localCap.NetValuePf = 0;
                                localCap.PfId = i;
                                localc.PfId = i;
                    ...
                    if (d == 0)
                    {
                        //capLocl = new List<CapPlan>();
                        //capLocl = CapPlansCurr.Where(b => (b.DatePlan.Date - Status.CurrTime.Date).Days == d).ToList();

                        List<int> loclpf = CapPlansCurr.Where(b => (b.DatePlan.Date - Status.CurrTime.Date).Days == d).Select(a => a.PfId).ToList();
                        ...
                        //lstCapNotCoil = lstCapPlan.Where(a => pfCoil.Contains(a.pfId) == false).ToList();
                        //foreach (var item in CapPlansNotCoil)
                        //{
                        //    CapPlansCurr.Remove(item);
                        //}
                        CapPlansCurr.RemoveAll(a => !pfCoil.Contains(a.PfId));
                    }
                }
            }
            CapPlansCurr = CapPlansCurr.OrderBy(a => a.DatePlan).ThenBy(a => a.PfId).ToList();

            #endregion CapPlansCurr

        }

        private void readMaxSetup()
        {
            dt = new DataTable();

            string comment = string.Format(@"select nvl(max(t.val_setup_cost_setut),0) as maxsetup
                                              from mas.mas_bas_setup_times t, mas_bas_program_type_defs p
                                             where p.bas_program_type_def_id = t.pgtyp_bas_program_type_def_id
                                               and p.statn_bas_station_id = {0}", RunInformation.NumStation);


            dt = WorkData.GetDataTable(comment, CommandType.Text);

            foreach (DataRow dr in dt.Rows)
                DataBase.MaxSetupCost = double.Parse(dr["maxsetup"].ToString());
        ...
        private void readSetup(List<ProgEfraz> ProgEfrazes, List<Setup> Setups)
        {
            dt = new DataTable();

            string comment = string.Format(@"select t.pgtyp_bas_program_type_def_id,
                                                   t.pgtyp_bas_program_type_duses_a,
                                                   t.dur_setup_time_setut,
                                                   t.val_setup_cost_setut
                                              from mas.mas_bas_setup_times t, mas_bas_program_type_defs p
                                              where p.bas_program_type_def_id=t.pgtyp_bas_program_type_def_id
                                              and p.statn_bas_station_id={0}", RunInformation.NumStation);
            ...
            foreach (ProgEfraz i in ProgEfrazes)
            {
                int from = i.IdEfraz;

                foreach (ProgEfraz j in ProgEfrazes)
                {
                    int to = j.IdEfraz;

                    List<Setup> loc = Setups.Where(a => a.IdProgFrom == from && a.IdProgTo == to).ToList();

                    if (loc.Count == 0)
                    {
                        Setup x = new Setup();
                        x.IdProgFrom = i.IdEfraz;
                        x.IdProgTo = j.IdEfraz;
                        x.CostSetup = 0;
                        x.DurTimeSetup = TimeSpan.Zero;
                        Setups.Add(x);
                    }
                }

                int fromIdShutDown = Parameter.IdShutDown;
                Setup sh = new Setup();
                sh.IdProgFrom = fromIdShutDown;
                sh.IdProgTo = from;
                sh.CostSetup = 0;
                sh.DurTimeSetup = TimeSpan.Zero;
                Setups.Add(sh);

                int fromIdInactive = Parameter.IdInactive;
                Setup shh = new Setup();
                shh.IdProgFrom = fromIdInactive;
                shh.IdProgTo = from;
                shh.CostSetup = 0;
                shh.DurTimeSetup = TimeSpan.Zero;
                Setups.Add(shh);

                int fromIdFinishCapDay = Parameter.IdFinishCapDay;
                Setup shhh = new Setup();
                shhh.IdProgFrom = fromIdFinishCapDay;
                shhh.IdProgTo = from;
                shhh.CostSetup = 0;
                shhh.DurTimeSetup = TimeSpan.Zero;
                Setups.Add(shhh);
            }
        ...
        private void readMaxValueGroup(List<MaxValueGroup> MaxValueGroups)
        {
            dt = new DataTable();
            string comment = string.Format(@"select t.order_group_id, t.qty_MAX_daily_ogprd, t.val_day_ogprd
                                          from lmp_cmp_prodcons_daily_viw t
                                         where t.bas_station_id ={0}
                                         and t.val_day_ogprd BETWEEN sysdate and sysdate+{1}
                                         and t.cod_group_type_ogtyp BETWEEN {2} AND {3}",
                                         RunInformation.NumStation, Parameter.DurationPlanning, Parameter.NumberStartGroupMaxValue
                                         , Parameter.NumberEndGroupMaxValue);

            dt = WorkData.GetDataTable(comment, CommandType.Text);

            foreach (DataRow dr in dt.Rows)
                addMaxValueGroup(dr, MaxValueGroups);
        }

        private void readEquipGroupFailureTime(List<EquipGroupFailureTime> EquipGroupFailureTimes)
        {
            dt = new DataTable();
            dt = WorkData.GetDataTable(@"select * from mas_cmp_equip_group_viw t
                                            where t.statn_bas_station_id={0}" + RunInformation.NumStation, CommandType.Text);
        ...
        private void readStationStops(List<StationStop> StationStops)
        {
            ...
            dt = new DataTable();
            dt = WorkData.GetDataTable(@"select t.COD_REPAIR_REPTM,
                                           t.date_repair,
                                           t.DUR_REPAIR_REPTM
                                      from mas_cmp_repair_time_viw t
                                    where t.statn_bas_station_id = {0}" + RunInformation.NumStation, CommandType.Text);
            foreach (DataRow dr in dt.Rows)
        ...
        private void readRoll(List<Roll> RollsWork, List<Roll> RollsBack, List<Coil> Coils)
        {
            ...
            string comment = string.Format(@" Select distinct (b.cod_roll_type_rolba),
        ...
        private void readWeightObjective()
        {
            ...
            string comment = string.Format(@"select t.val_multi_value_cofal, t.lkp_multi_cofal
...
//        }
        ...
        public void addSetup(DataRow dr, List<Setup> Setups)
        {
            Setup b = new Setup(int.Parse(dr["PGTYP_BAS_PROGRAM_TYPE_DEF_ID"].ToString()), int.Parse(dr["PGTYP_BAS_PROGRAM_TYPE_DUSES_A"].ToString()),
                double.Parse(dr["VAL_SETUP_COST_SETUT"].ToString()), TimeSpan.Parse(dr["DUR_SETUP_TIME_SETUT"].ToString()));
